...
from mmdet.models.roi_heads.mask_heads.fcn_mask_head import BYTES_PER_FLOAT, GPU_MEM_LIMIT, _do_paste_mask
...
class WaterMaskHead(BaseModule):
    ...
    def __init__(self,
                 ...
                 init_cfg=None):
        super(WaterMaskHead, self).__init__(init_cfg=init_cfg)
        ...
        self.loss_lcf = MODELS.build(loss_cfg)
    ...
    def loss_and_target(self,
                        ...
                        batch_gt_instances,
                        rcnn_train_cfg=None):
        """MMDet3-compatible loss API.

        This method is added for compatibility with the standard
        ``StandardRoIHead.mask_loss`` workflow.
        """
        del rcnn_train_cfg
        ...
        stage_lcf_targets = self.get_targets(
            pos_proposals, pos_assigned_gt_inds, gt_masks)
        if isinstance(mask_preds, Tensor):
            stage_lcf_preds = [mask_preds]
        else:
            stage_lcf_preds = list(mask_preds)

        loss_mask = self.loss(stage_lcf_preds, stage_lcf_targets)
        return dict(loss_mask=loss_mask, mask_targets=stage_lcf_targets)
    ...
    def predict_by_feat(self,
        ...
        """MMDet3-compatible prediction API.

        ``mask_preds`` should be split by image before passing in.
        """
        del activate_map
        ...
        for img_id in range(len(batch_img_metas)):
            ...
            results.masks = self._segms_to_instance_masks(
                segms, results.labels, bboxes.device)
    ...
    def _segms_to_instance_masks(self, cls_segms: List[list], labels: Tensor,
        ...
        for label in labels.detach().cpu().tolist():
            ordered_masks.append(
                torch.from_numpy(cls_segms[label][class_cursor[label]]))
    ...
    def get_seg_masks(self, mask_pred, det_bboxes, det_labels, rcnn_test_cfg, ori_shape, scale_factor, rescale):
        ...
        # In mmdet3, scale_factor may be float / tuple / list / ndarray / Tensor
        if isinstance(scale_factor, torch.Tensor):
            scale_factor = scale_factor.to(det_bboxes.device)
        else:
            if isinstance(scale_factor, float):
                scale_factor = np.array([scale_factor] * 4, dtype=np.float32)
                warn('Scale_factor with float type is deprecated; '
                     'prefer a sequence with shape (2,) or (4,).')
                ...
                scale_factor = scale_factor.astype(np.float32, copy=False)
            else:
                raise TypeError(
                    f'Unsupported scale_factor type: {type(scale_factor)}')
            scale_factor = torch.from_numpy(scale_factor).to(det_bboxes.device)

        if scale_factor.numel() == 2:
            scale_factor = scale_factor.repeat(2)
        elif scale_factor.numel() != 4:
            raise ValueError(
                f'scale_factor should have 2 or 4 elements, got '
                f'{scale_factor.numel()}')

        if rescale:
            img_h, img_w = ori_shape[:2]
            det_bboxes[:, :4] /= scale_factor.to(det_bboxes)
        else:
